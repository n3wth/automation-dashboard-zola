...
const createStagehandConfig = (): ConstructorParams => ({
  env: 'BROWSERBASE',
  verbose: 1,
...
class EfficientPRWorkflow {
  ...
  async initialize(): Promise<void> {
    ...
    this.stagehand = new Stagehand(createStagehandConfig());
  ...
  private async navigateToCodex(): Promise<void> {
    if (!this.stagehand?.page) throw new Error('Stagehand not initialized');
  ...
  private async performAutomatedLogin(): Promise<void> {
    if (!this.stagehand?.page) throw new Error('Stagehand not initialized');
  ...
  private async checkIfLoginRequired(): Promise<boolean> {
    if (!this.stagehand?.page) return true;
  ...
  private async verifyCodexAccess(): Promise<boolean> {
    if (!this.stagehand?.page) return false;
  ...
  private async submitTask(issue: GitHubIssue): Promise<string> {
    if (!this.stagehand?.page) throw new Error('Stagehand not initialized');
  ...
  private async createPRForTask(issue: GitHubIssue): Promise<string | undefined> {
    if (!this.stagehand?.page) throw new Error('Stagehand not initialized');
  ...
  private async processBatch(issues: GitHubIssue[]): Promise<void> {
  ...
  async processIssues(
    options: {
      maxIssues?: number;
      priorityFilter?: GitHubIssue['priority'][];
      categoryFilter?: GitHubIssue['category'][];
      batchSize?: number;
    } = {}
...
`;
  ...
  private generatePriorityBreakdown(): string {
...
export { runEfficientWorkflow, EfficientPRWorkflow, type GitHubIssue, type WorkflowResult, type WorkflowStats };
