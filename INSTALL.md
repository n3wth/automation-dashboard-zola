# Bob Installation Guide

Bob is a free, open-source AI chat app with multi-model support. This guide covers how to install and run Bob on different platforms, including Docker deployment options.

![Bob screenshot](./public/cover_bob.webp)
...
Bob supports BYOK functionality, allowing users to securely store and use their own API keys for AI providers. To enable this feature, you need to configure an encryption key for secure storage of user API keys.
...
Ollama allows you to run AI models locally on your machine. Bob has built-in support for Ollama with automatic model detection.
...
### Bob + Ollama Integration

Bob automatically detects all models available in your Ollama installation. No additional configuration is needed!

**Features:**

- **Automatic Model Detection**: Bob scans your Ollama instance and makes all models available
...
By default, Bob connects to Ollama at `http://localhost:11434`. This works for local installations.
...
Bob includes a settings interface where you can:
...
For a complete Docker setup with both Bob and Ollama:
...
docker run -d -v ollama:/root/.ollama -p 11434:11434 --name ollama ollama/ollama
docker run -p 3000:3000 -e OLLAMA_BASE_URL=http://ollama:11434 bob
```
...
1. Refresh the models list in Bob settings
2. Check Ollama has models: `ollama list`
3. Restart Bob if models were added after startup
...
Bob requires Supabase for authentication and storage. Follow these steps to set up your Supabase project:
...
For a complete setup with both Bob and Ollama running locally, use the provided `docker-compose.ollama.yml`:

```bash
# Start both Bob and Ollama services
...
- **Proper networking** between Bob and Ollama
- **Volume persistence** for Ollama models

The Ollama service will be available at `http://localhost:11434` and Bob will automatically detect all available models.
...
The easiest way to deploy Bob is using Vercel:
...
You can customize various aspects of Bob by modifying the configuration files:
